...
async function createNotification(req,data){
	...
	const result = await req.db.collection('notifications').insertOne(notification);
...
async function getdashboarddatas(db){
...
async function emitDashboardUpdate(req){
	const dashboardDatas = await getdashboarddatas(req.db);

	req.io.to('dashboard').emit("dashboard:update",dashboardDatas);
}
	...
	try{
		...
		if(!aboutpage){
			const defaultAbout = {
				slug: 'about',
				title: 'About Page',
				body: "This is the default about page content.",
				imageurls: [],
				createdAt: new Date(),
				updatedAt: new Date()
			}

			const result = await req.db.collection('pages').insertOne(defaultAbout);
			aboutpage = {_id: result.insertId,...defaultAbout};
		}
...
app.get('/about/edit',isAuth, async (req, res) => {
	try{
		...
		res.status(500).render('error',{
			title: "Server Error",
			message: "Failed to load about about page"
		})
	}
})
...
app.post('/about/edit',isAuth, upload.array('images',5), async (req, res) => {
	try{
		...
		let aboutpage = await req.db.collection('pages').findOne({slug:"about"});
		if(!aboutpage) return res.status(404).render('404',{title: "404"});
		if(!title || !body){
			return res.render('aboutedit',{
				title: 'Edit About Page',
		...
		// own method 2 HW (Old image deleted, only new image keep)
		...
		if(req.files && req.files.length > 0){
			// method 1 combine old + new
			// const newimageUrls = req.files.map(file=> `/uploads/${file.filename}`);
			// imageurls = [...imageurls,...newimageUrls]

			// methd 2 remove old, add new
			for(const oldimg of imageurls){
				// remove extra /
				const oldpathsafe = path.join(__dirname,"public",oldimg.replace( /^\/+/ ,""));
				fs.unlink(oldpathsafe,(err)=>{
					if (err) console.log("OLd about image delete error:",err.message);
				});
			}
		...
		await req.db.collection('pages').updateOne(
			{slug: "about"},
			{$set: updateData}
		)
...
app.post("/posts/:slug/like",isAuth,async(req,res)=>{
	...
	try{
		...
		const alreadyLiked = post.likes?.some(id => id.toString() === userid.toString()); //return true / false
		if(alreadyLiked){
			// remove like 
		...
		const payload = {
			slug,
			likesCount: updatePost.likes?.length || 0,
			dislikesCount: updatePost.dislikes?.length || 0,
			liked: updatePost.likes?.some(id => id.toString() === userid.toString()) || false,
			disliked: updatePost.dislikes?.some(id => id.toString() === userid.toString()) || false,
		}
...
app.post("/posts/:slug/dislike",isAuth,async(req,res)=>{
	...
	try{
		...
		const alreadyDisliked = post.dislikes?.some(id => id.toString() === userid.toString()); //return true / false
		if(alreadyDisliked){
			// remove dislike 
		...
		return res.status(500).json({
			success: false,
			error: `Failed to disike post: ${error.message}`
		})
	}
});
...
// Start Notifications
	...
	app.get("/notifications",isAuth,async(req,res)=>{
		try{
			...
			const notifications = await req.db.collection('notifications').find({userId}).sort({createdAt: -1}).limit(50).toArray();
			...
			console.error("Notofication error: ",error);
			return res.status(500).render("error",{
				title: "Server Error",
				error: `Failed to load notification: ${error.message}`
			});
		}
	});
	app.get('/notifications/count',isAuth,async(req,res)=>{
		try{
			...
			return res.status(500).json({
				success: false,
				message: `Failed to load notification: ${error.message}`
			});
		}
	});
...
app.post("/posts/:id/delete",isAuth,async(req,res)=>{
	try{
		...
		const existing = await req.db.collection("posts").findOne({_id: new ObjectId(id)});
		if(!existing) res.status(404).render('404',{title: "404"});
		...
		if(existing.imageURL){
			...
			fs.unlink(oldpathsafe,(err)=>{
				if(err){
					...
					console.log("Deleted image: ",existing.imageURL);
				}
			})
		}
...
// arrys.some(array=>(condition)); 

// const nums = [10,20,30,40,50];
// const result = nums.some(num=>num > 25); 

// Iteration 1: 10 > 25 = false
// Iteration 2: 20 > 25 = false
// Iteration 3: 30 > 25 = true
// Iteration stop immediately = true
...
// Yes — exactly 👍
// req.file and req.files are created/filled by Multer automatically.
...
// 👉 Multer becomes a middleware in your route:

// app.post('/posts/create', upload.single('image'), async (req,res)=>{ ... })
// ⚙️ Request flow (important)
// Browser sends request (multipart/form-data)
// Express receives it
// 👉 Multer runs BEFORE your controller
// Multer:
// reads the request
// extracts files
// saves them (disk/memory)
// adds new properties to req
// THEN your route handler runs
// 🧠 So yes — Multer “updates” req

// It mutates the request object like this:

// ✅ Case 1: upload.single('image')
